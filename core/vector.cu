...
__device__ __host__ const float4 saturate_(const float4 v) { return { saturate_(v.x),saturate_(v.y),saturate_(v.z),saturate_(v.w) }; }
__device__ __host__ const float3 max(const float3 a, const float3 b) { return { max(a.x, b.x) , max(a.y, b.y), max(a.z, b.z) }; }
__device__ __host__ const float4 max(const float4 a, const float4 b) { return { max(a.x, b.x) , max(a.y, b.y), max(a.z, b.z), max(a.w, b.w) }; }
__device__ __host__ const float3 min(const float3 a, const float3 b) { return { min(a.x, b.x) , min(a.y, b.y), min(a.z, b.z) }; }
__device__ __host__ const float2 max(const float2 a, const float2 b) { return { max(a.x, b.x) , max(a.y, b.y) }; }
__device__ __host__ const float2 min(const float2 a, const float2 b) { return { min(a.x, b.x) , min(a.y, b.y) }; }
