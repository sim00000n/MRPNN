...
void Camera::RenderToFile(string path, float3 lightDir, float3 lightColor, float alpha, int multiScatter, float g, int sampleNum, ToneMapFunction tone, VolumeRender::RenderType rt, float exp) {
    ...
    printf("%s done in %.2f s.\n", path.c_str(), (clock() - start_time) / 1000.0f);

    for (int i = 0; i < resolution; i++)
    {
        for (int j = 0; j < resolution; j++)
        {
            float4 res = ress[i * resolution + j];//tone(ress[i * resolution + j] * exp);
            res = saturate_(res);
            data[(i * resolution + j) * 4] = res.x * 255;
            data[(i * resolution + j) * 4 + 1] = res.y * 255;
            data[(i * resolution + j) * 4 + 2] = res.z * 255;
    ...
    stbi_write_png((path + ".png").c_str(), resolution, resolution, 4, data, resolution * 4);
...
void Camera::GatherData(float* __restrict__ out_buffer, int2 size, float3 lightDir, float alpha, float g, int2 pixelOffset) {
    volume->GatherData(out_buffer, size, ori, up, right, lightDir, alpha, g, pixelOffset);
}