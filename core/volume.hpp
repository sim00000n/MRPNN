...
class VolumeRender {
	...
	cudaArray* datas_dev = nullptr;
	cudaArray* datas_devL = nullptr;
	cudaArray* datas_devS = nullptr;
	//cudaArray* datas_dev16u = 0;
	cudaChannelFormatDesc channel_desc;
	cudaChannelFormatDesc channel_desc16u;
	cudaExtent size;
	...
	void MallocMemory(bool cachedTR = false);
	void Update_CachedTR(float3 lightDir, float alpha);
...
public:
	...
	VolumeRender(string path, bool cacheTR = false);
	~VolumeRender();

	void LoadData(string path, bool cacheTR);

	void SetData(int x, int y, int z, float value);

	void SetDatas(FillFunc func);

	void Update(bool process_mips = true, bool need_reset_datas_dev = true);
	...
	vector<float4> Render(int2 size, float3 ori, float3 up, float3 right, float3 lightDir, RenderType rt = RenderType::PT, float g = 0.857, float alpha = 1, float3 lightColor = { 1, 1, 1 }, int multiScatter = 512, int sampleNum = 1024);
	...
	void GatherData(float* __restrict__ out_buffer, int2 size, float3 ori, float3 up, float3 right, float3 lightDir, float alpha = 1, float g = 0, int2 pixelOffset = { 0, 0 }, int randseed = 0);
};