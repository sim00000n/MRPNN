...
__device__ __host__ const float3 operator+(const float3 a, const float b);
__device__ __host__ const float3 operator+(const float3 a, const float3 b);
__device__ __host__ inline const float2 operator+(const float2 a, const float b) { return { a.x + b,a.y + b }; }
...
__device__ __host__ inline float2& operator+=(float2& a, const float b) {
	a.x += b;
	a.y += b;
	return a;
}
...
__device__ __host__ const float4 saturate_(const float4 v);
__device__ const float3 saturate(const float3 v);
__device__ __host__ const float3 max(const float3 a, const float3 b);
__device__ __host__ const float4 max(const float4 a, const float4 b);
__device__ __host__ const float3 min(const float3 a, const float3 b);
__device__ __host__ const float2 max(const float2 a, const float2 b);
__device__ __host__ const float2 min(const float2 a, const float2 b);
