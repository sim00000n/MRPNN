...
__device__ float3 RadiancePredictInternal(float* X_Val, float* X_Val_Sub, float* X_Val_Hg, float gamma, float g, float3 scatterrate)
{
    const int DI = 160;
    const int POOL = 36;
    const int P_DI = 24;
...
// Temp, remove later
__device__ void SetupInputData(bool active, const float3& pos, const float3& XMain, const float3& YMain, const float3& ZMain, const float3& LXMain, const float3& LightDir, float g, float alpha, float* out_X_Val, float* out_X_Val_Hg, float* out_X_Val_Sub, float& out_gamma)
{
    int randIndex = 0;
    ...
    if (active) {
        ...
        for (int i = 0; i < PC; i++)
        {
            ...
            out_X_Val[i] = log(1.0f + alpha * MipDensityDynamic(int(CurrentOffsetInfo.Layer), CurrentOffsetPos) / 64.0f);

            float3 MsDir;
            if (CurrentOffsetInfo.localindex == 0)
            {
                MsDir = XMain;
                out_X_Val_Hg[i] = log(HenyeyGreenstein(dot(MsDir, LightDir), g) + 1.0f);
            }
            else
            {
                ...
                out_X_Val_Hg[i] = log(HG0 * HG1 + 1.0f);
            }
            //X_Val_Sub[i] = TR_MipDensityDynamic(max(int(CurrentOffsetInfo.Layer) - 1, 0), CurrentOffsetPos);
            out_X_Val_Sub[i] = ShadowTerm_TRTex(CurrentOffsetPos, LXMain, XMain, float3{ 1.0f,1.0f,1.0f }, g, max(int(CurrentOffsetInfo.Layer) - 1, 0)).x;
        }
    }
    else {
        // fill with fake data
    ...
    float d = dot(XMain, LXMain);
    d = fminf(1.0f, fmaxf(-1.0f, d));
    out_gamma = acos(d);
}

__device__ void SetupInputData2(const float3 ori, const float4 samples[16], const float3& XMain, const float3& YMain, const float3& ZMain, const float3& LightDir, float g, float alpha, float* out_X_Val, float* out_X_Val_Hg, float* out_X_Val_Sub)
{
    for (int i = 0; i < 16; i++)
    {
        ...
        if (sample.w)
        {
            float3 samp_dir = make_float3(sample);
            float3 samp_pos = ori + samp_dir * sample.w;

            out_X_Val[i] = log(1.0f + alpha * MipDensityDynamic(0, samp_pos) / 64.0f);
            out_X_Val_Hg[i] = log(HenyeyGreenstein(dot(samp_dir, LightDir), g) + 1.0f);
            out_X_Val_Sub[i] = ShadowTerm_TRTex(samp_pos, LightDir, samp_dir, float3{1.0f,1.0f,1.0f}, g, 0).x;
        }
        else
        {
            out_X_Val[i] = 0;
    ...
    for (int i = 16; i < PC; i++)
    {
        for (; !samples[samp_id].w; samp_id = (samp_id + 1) & 0xf);

        const float4& sample = samples[samp_id];
        samp_id = (samp_id + 1) & 0xf;

        float3 samp_pos = ori + make_float3(sample) * sample.w;

        Offset_Layer_ CurrentOffsetInfo = GetSamples23_(i);
        float3 CurrentOffsetPos;
        if (CurrentOffsetInfo.type >= 4)
        {
            CurrentOffsetPos = samp_pos + SphereRandom3(i, CurrentOffsetInfo.Offset, XMain, YMain, ZMain, g);
        }
        else
        {
            CurrentOffsetPos = samp_pos + LightDir * CurrentOffsetInfo.Offset;
        }

        out_X_Val[i] = log(1.0f + alpha * MipDensityDynamic(int(CurrentOffsetInfo.Layer), CurrentOffsetPos) / 64.0f);

        float3 MsDir = normalize(CurrentOffsetPos - samp_pos);
        float Radius = float(1 << int(CurrentOffsetInfo.Layer)) / 256.0f;
        float Angle = atan(0.5f * Radius / CurrentOffsetInfo.Offset);
        float cos = dot(XMain, MsDir);
        float cos2 = dot(LightDir, MsDir);
        float u = cos * 0.5f + 0.5f;
        float u2 = cos2 * 0.5f + 0.5f;
        float v = Angle / (3.1415926535f * 60.0f / 180.0f);
        float HG0 = tex2D<float>(_HGLut, u, v);
        ...
        //X_Val_Sub[i] = TR_MipDensityDynamic(max(int(CurrentOffsetInfo.Layer) - 1, 0), CurrentOffsetPos);
        out_X_Val_Sub[i] = ShadowTerm_TRTex(CurrentOffsetPos, LightDir, XMain, float3{1.0f,1.0f,1.0f}, g, max(int(CurrentOffsetInfo.Layer) - 1, 0)).x;
    }
}
...
__device__ float3 RadiancePredict(curandState* seed, bool active, float3 pos, float3 LightDir, float3 XMain, float3 YMain, float3 ZMain, float3 LXMain, float3 LYMain, float3 LZMain, float alpha, float g, float3 scatterrate)
{
    ...
    SetupInputData(active, pos, XMain, YMain, ZMain, LXMain, LightDir, g, alpha, X_Val, X_Val_Hg, X_Val_Sub, gamma);

    return RadiancePredictInternal(X_Val, X_Val_Sub, X_Val_Hg, gamma, g, scatterrate);
}
...
{
    SetupInputData2(ori, samples, XMain, YMain, ZMain, LightDir, g, alpha, out_buffer, out_buffer + PC, out_buffer + PC * 2);
}
